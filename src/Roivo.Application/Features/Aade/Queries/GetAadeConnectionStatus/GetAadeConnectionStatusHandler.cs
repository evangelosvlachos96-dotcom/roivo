...
public sealed class GetAadeConnectionStatusHandler
{
    ...
    public async Task<AadeConnectionInfo> Handle(GetAadeConnectionStatusQuery query, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(query);

        var business = await _businesses.GetByIdActiveOnlyAsync(query.BusinessId, cancellationToken).ConfigureAwait(false);
        if (business is null)
            return new AadeConnectionInfo(IsConnected: false, LastSyncAt: null, MaskedUserId: null);

        var creds = await _credentials.RetrieveAsync(business.Id, cancellationToken).ConfigureAwait(false);
        if (creds is null)
            return new AadeConnectionInfo(IsConnected: false, LastSyncAt: business.LastAadeSyncAt, MaskedUserId: null);

        return new AadeConnectionInfo(
            IsConnected: true,
